...
(function($){
    ...
    var PageResize = function ( dt )
    {
        ...
        this.s = {
            dt:             dt,
            host:           $(table.container()).parent().parent().parent(),
            header:         $(table.header()),
            footer:         $(table.footer()),
            body:           $(table.body()),
            container:      $(table.container()),
            table:          $(table.node()),
            layoutHeader:   $($(table.container()).parent().parent().parent().children()[0]),
            layoutFooter:   $($(table.container()).parent().parent().children()[1])
        };
    ...
    PageResize.prototype = {
        _size: function ()
        {
            var settings = this.s;
            var dt = settings.dt;
            var drawRows = this._calculate();

            if ( drawRows !== Infinity && drawRows !== -Infinity &&
                ! isNaN( drawRows )   && drawRows > 0 &&
                drawRows !== dt.page.len()
            ) {
                dt.page.len( drawRows ).draw(false);
            }
        },

        _calculate: function () {
            var settings = this.s;
            var t = settings.dt.table();
            var offsetTop = $(settings.table).offset().top;
            if ($('tr', settings.body).eq(0).height() != 0)
                var rowHeight = $('tr', settings.body).eq(0).height() + 0.5;
            else
                var rowHeight = 33.5;
            var availableHeight = settings.host.height();
            var scrolling = t.header().parentNode !== t.body().parentNode;
            var layoutHeader = settings.layoutHeader.height() + 7;
            var layoutFooter = settings.layoutFooter.height() + 5;
            ...
            availableHeight -= layoutHeader;
            availableHeight -= layoutFooter;
