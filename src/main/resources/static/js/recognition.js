...
function ArchivTyp(srch, parentType) {
        ...
        if (REC.errors.length == 0 && documente.length > 0) {
            ...
            for (var i = 0; i < documente.length; i++) {
                ...
                if (this.unique == "newVersion") {
                    ...
                    REC.log(ERROR, "Dokument mit dem " + (REC.currentDocument.name == document.name ? "Dateinamen " + REC.currentDocument.name : "Titel " + REC.currentDocument.properties["cm:title"]) + " ist im Zielordner bereits vorhanden! ");
    ...
    this.resolve = function () {
        ...
        if (REC.errors.length > 0) {
            ...
            REC.log(TRACE, "adding Comment " + comment);
            var COM = new Comments();
            ...
            if (!REC.currentDocument.move(REC.errorBox))
                ...
                REC.log(INFORMATIONAL, "Errors ocured, document successfuly moved to " + REC.completeNodePath(REC.errorBox));
        }
    ...
    moveDocToUnknownBox: function (doc) {
        ...
        if (REC.exist(erg)) {
            ...
            for (var j = 0; j < erg.length; j++) {
                ...
                if (erg[j].val < compareDate && (!REC.exist(foundDate) || Math.abs(foundDate - compareDate) > Math.abs(erg[j].val - compareDate)))
                    foundDate = erg[j].val;
            }
            // Foldernamen bilden
            var folderName;
